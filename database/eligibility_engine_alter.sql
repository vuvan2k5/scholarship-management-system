...
ALTER TABLE eligibility_results
    ADD COLUMN rule_trace JSON DEFAULT NULL
        COMMENT 'JSON array of per-rule evaluation detail'   AFTER reason,
    ADD COLUMN checked_by INT(11) DEFAULT NULL
        COMMENT 'FK → users.id (admin who triggered the run)' AFTER rule_trace,
    ADD CONSTRAINT fk_er_checked_by
